...
function collision.rectanglevector(left,top,width,height,  vx,vy,vx2,vy2)
	local y1,y2,x1,x2
	if vx - vx2 < 0 then
		if vx < left+width and vx > left then
			if vy < top + height/2 and vy2 > top+height/2 or (vy2 < top + height/2 and vy2 < top + height/2) then
				return true
			end
		end
	end
	local m = (vy-vy2)/(vx-vx2)
	local b = vy-m*vx
	y1 = m*left + b
	y2 = m*(left+width) + b
	...
	x1 = (top-b)/m
	x2 = (top+height-b)/m
	return (y1 > top and y1 < top + height) or (y2 > top and y2 < top + height)
end
